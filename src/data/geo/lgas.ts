/**
 * Local Government Areas (LGAs) of Nigeria, mapped to their state.
 *
 * These are the real, official LGA names for each of the 36 states + FCT
 * (the 774 LGAs). They are used as realistic mock administrative data today and
 * are structured so a real dataset (e.g. with LGA codes, population, geometry)
 * can replace this map without touching the UI — keep the `Record<state, string[]>`
 * shape, or extend each entry to an object and adjust the helpers in calculations.ts.
 */
